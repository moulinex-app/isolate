...
static const struct cg_controller_desc
    cg_controllers[CG_NUM_CONTROLLERS + 1] = {
        [CG_MEMORY] = { "memory", 0 },
...
void cg_remove(void)
{
    ...
    FOREACH_CG_CONTROLLER(controller)
    {
        // The cgroup can be non-existent at this moment (e.g., --cleanup before
        // the first --init)
        if (!cg_read(controller, "?tasks", buf))
            continue;

        if (buf[0])
            die("Some tasks left in controller %s of cgroup %s, failed to "
                "remove it",
