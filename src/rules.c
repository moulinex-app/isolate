...
static int set_dir_action_ext(char *arg, unsigned int ext_flags)
{
    ...
    else if (flags & DIR_FLAG_TMP)
    {
         ...
         *  chowned to box_uid. The hierarchy of these directories is
         * intentionally flat, so that we avoid writing to a directory which
         * might have already tampered with in a previous run of the sandbox.
         */
        char out[1024];
...
void apply_dir_rules(int with_defaults)
{
    ...
    for (struct dir_rule *r = first_dir_rule; r; r = r->next)
    {
        ...
        if (r->flags & DIR_FLAG_FS)
        {
            ...
            if (!strcmp(in, "proc"))
            {
                // If we are mounting procfs, add hidepid=2, so that only the
                // processes of the same user are visible. This has to be done
                // as a remount.
                if (mount("none", root_in, out, MS_REMOUNT | mount_flags,
                          "hidepid=2")
                    < 0)
        ...
        else
        {
            ...
            /*
             *  This is tricky. We cannot run mount() with root privileges,
             * since it could be used to bypass access control if the mounted
             * path contains elements inaccessible to the user running isolate.
             *
             *  We switch effective UID and GID back to the calling user (which
             * clears all capabilities, but keeps them in the permitted set) and
             * then enable CAP_SYS_ADMIN. So we have CAP_SYS_ADMIN (needed for
             * mount), but not CAP_DAC_OVERRIDE (which allows to bypass
             * permission checks).
             */
            ...
            if (mount(out, root_in, "none", mount_flags, "") < 0
                || mount(out, root_in, "none", MS_REMOUNT | mount_flags, "")
                    < 0)
                die("Cannot mount %s on %s: %m", out, in);
...
void set_quota(void)
{
    ...
    if (cwd_st.st_dev != dev_st.st_rdev)
        die("Identified %s as a filesystem on %s, but it is obviously false",
            cwd, dev);
